...
axis( 2, at = 1:61, labels = lastnt[rev(nonstop)], pos = codonrange1, hadj=0.5, padj=0.5, las = 1, lwd = 0.75, tick = F, cex.axis = 0.6, family="mono" )
axis( 2, at = 1:61, labels = firstnt[rev(nonstop)], pos= codonrange1 - 0.75, hadj=0.85, padj=0.7, las = 1, lwd = 0.75, tick = F, cex.axis = 1, family="mono" )
...
axis( 1, at = codonrange1:codonrange2, labels = NA, tick = T, lwd = 0, lwd.ticks = 0.5)
...
# diagonal lines
par(xpd=NA)
segments(  seq(codonrange1*1.5, codonrange2*1.5, by=1.5), -0.8,
           codonrange1:codonrange2, -0.05,
           lwd = 0.5
)

...
if [ ! -f $GENOME_DIR/human.transcripts.13cds10.1.ebwt ]; then
	bowtie-build $GENOME_DIR/human.transcripts.13cds10.fa $GENOME_DIR/human.transcripts.13cds10
fi
if [ ! -f $GENOME_DIR/human.transcripts.13cds10.idx.fa ]; then
	rsem-prepare-reference $GENOME_DIR/human.transcripts.13cds10.fa $GENOME_DIR/human.transcripts.13cds10
fi

cat SI1DMSO1.fastq SI1DMSO2.fastq | ./trim_linker.pl > SI1DMSO1_02.trimmed.fastq

~/bowtie-1.2.1.1/bowtie -v 2 -p 36 -S --un $IWASAKI_PROC_DIR/iwasaki.not_rrna_trna.fastq \
	$GENOME_DIR/human_rrna_trna \
	$IWASAKI_PROC_DIR/SI1DMSO1_02.trimmed.fastq > $IWASAKI_PROC_DIR/iwasaki.rrna_trna.sam 2> $IWASAKI_PROC_DIR/iwasaki.rrna_trna.bowtiestats
